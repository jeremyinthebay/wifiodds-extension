...
Missing evidence is unknown. Do not convert it to 0%, last place, or "worse than scored flights." Unscored airlines stay unscored and keep their relative order.

### Next-gen odds collapsed into Streaming score

Keep the two numbers apart:

- Per-flight next-gen odds: historical evidence for one United or Alaska flight when a tracker publishes it.
- Streaming score: 0-100 rating of an airline's WiFi across its fleet today. It does not predict the WiFi on a specific flight.

### Store-copy brand catalogs

A comma-separated airline dump in Chrome Web Store fields is keyword stuffing, not a product description. Name the surfaces the listing is actually describing.

### Unicode dash as a pause

U+2014 and U+2013 are banned in this repo, including in "after" examples. Use a comma, colon, hyphen-minus, or period.
